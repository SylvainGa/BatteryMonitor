...
- Redid completely how the history is stored because Glance mode can work with far less data than the app (on my Fenix 7S Pro) can work with and will crash once over 700 elements are stored. Now it has room for 5 arrays of 500 elements each. Only the last array is dealt with in Glance and once an array is filled, its slopes becomes static and don't need to be recalculated. This also improves efficiencies. The graph efficiency has been improved on skipping data when there is more data to display than pixels.
